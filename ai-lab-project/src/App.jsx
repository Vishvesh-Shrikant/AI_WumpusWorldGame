...
export default function App() {
    ...
    } catch (err)
 {
  ...
  const handleNextTurn = useCallback(async () => {
    ...
    setMessage({ text: '', type: 'info' }); // Clear previous messages
    try {
      const res = await fetch(`${API_URL}/${game._id}/turn`, { method: 'POST' });
      const data = await res.json();
      
      if (!res.ok) {
        const errorData = { message: data.msg || 'Failed to take turn', game: data.game };
        throw errorData;
      }
  ...
  const getMessageColor = () => {
    if (message.type === 'success') return 'bg-green-500/10 text-green-300 border-green-500/30';
    if (message.type === 'error') return 'bg-red-500/10 text-red-300 border-red-500/30';
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-900 px-4 py-4 sm:py-5 font-sans text-gray-200">
      <div className="container w-full max-w-3xl bg-gray-800 shadow-2xl rounded-2xl p-4 md:p-5 max-h-[95vh] flex flex-col">
        ...
        {message.text && (
          <div className={`p-3 rounded-lg my-4 border text-sm font-medium ${getMessageColor()}`}>
            {message.text}
          </div>
        )}
        ...
        {!game && (
          <div className="text-center py-16">
            <h2 className="text-2xl font-semibold text-gray-400 mb-6">Welcome, KBA.</h2>
            <button 
              onClick={startNewGame} 
              disabled={isLoading}
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-5 gap-4 min-h-0">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              ...
              <div className="text-left bg-gray-900/50 p-3 rounded-xl border border-gray-700">
                <h3 className="text-base font-bold text-white mb-2">Game Status</h3>
                <div className="space-y-1">
                  <p className="text-xs">Move: <strong className="font-bold text-gray-100 ml-2">{game.moveCount}</strong></p>
                  <p className="text-xs">Optimizer Temp: <strong className="font-bold text-gray-100 ml-2">{game.optimizer_temp.toFixed(2)}</strong></p>
                </div>
                
                {game.gameOver && (
                  <div className={`text-base font-bold mt-3 p-2 rounded-lg text-center ${game.gameWon ? 'bg-green-500/10 text-green-300' : 'bg-red-500/10 text-red-300'}`}>
